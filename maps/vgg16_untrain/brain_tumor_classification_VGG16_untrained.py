...
output_file = 'maps/vgg16_untrain/brain_tumor_classification_VGG16_untrained.txt'
...
def compute_grad_cam(model, img_tensor, target_class_idx=None):
    ...
    model.features[-2].register_forward_hook(hook)  # Assuming the last convolutional layer is model.features[-2]
...
for image_path, target in image_paths:
    ...
    os.makedirs("maps/vgg16_untrain", exist_ok=True)

    output_image_path = f"maps/vgg16_untrain/{os.path.basename(image_path).split('.')[0]}_gradcam_output.png"
