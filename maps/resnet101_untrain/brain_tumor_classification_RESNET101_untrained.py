...
output_file = 'maps/resnet101_untrain/brain_tumor_classification_RESNET101_untrained.txt'
...
def compute_grad_cam(model, img_tensor, target_class=None):
    ...
    hook_handle = model.layer4[-1].conv3.register_forward_hook(hook)  # Assuming the last convolutional layer is layer4[-1].conv3
...
for image_path in image_paths:
    ...
    os.makedirs("maps/resnet101_untrain", exist_ok=True)

    output_image_path = f"maps/resnet101_untrain/{os.path.basename(image_path).split('.')[0]}_gradcam_output.png"
