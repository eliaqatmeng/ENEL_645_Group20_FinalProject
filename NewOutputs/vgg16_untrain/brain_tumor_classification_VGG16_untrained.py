...
output_file = 'NewOutputs/vgg16_untrain/brain_tumor_classification_VGG16_untrained.txt'
...
def plot_roc_curve_and_output_auc(labels, probs, filename_png='NewOutputs/vgg16_untrain/ROC_curve_VGG16_untrained.png', filename_csv='NewOutputs/vgg16_untrain/ROC_data_VGG16_untrained.csv'):
...
def compute_grad_cam(model, img_tensor, target_class=None):
    ...
    hook_handle = model.features[-1].register_forward_hook(hook)  # Assuming last convolutional layer is model.features[-1]
...
for epoch in range(num_epochs):
    ...
    if epoch == num_epochs - 1:
        ...
        plot_grad_cam_for_image(model, image_path=sample_image_path51, actual_class=0, target_class=0, filename='NewOutputs/vgg16_untrain/grad_cam51.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path2419, actual_class=1, target_class=1, filename='NewOutputs/vgg16_untrain/grad_cam2419.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path2373, actual_class=1, target_class=1, filename='NewOutputs/vgg16_untrain/grad_cam2373.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path939, actual_class=0, target_class=0, filename='NewOutputs/vgg16_untrain/grad_cam939.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path331, actual_class=0, target_class=0, filename='NewOutputs/vgg16_untrain/grad_cam331.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path370, actual_class=1, target_class=1, filename='NewOutputs/vgg16_untrain/grad_cam370.png')


    # Save Confusion Matrix as Image after the last epoch
    if epoch == num_epochs - 1:
        plot_confusion_matrix(conf_matrix, classes=[str(i) for i in range(2)], filename='NewOutputs/vgg16_untrain/confusion_matrix_vgg16_untrained.png')
