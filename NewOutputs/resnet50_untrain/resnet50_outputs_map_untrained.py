...
output_file = 'NewOutputs/resnet50_untrain/RESNET50_untrained_gradcam.txt'
...
def plot_roc_curve_and_output_auc(labels, probs, filename_png='NewOutputs/resnet50_untrain/ROC_curve_resnet50_untrained.png', filename_csv='NewOutputs/resnet50_untrain/ROC_data_resnet50_untrained.csv'):
...
def compute_grad_cam(model, img_tensor, target_class=None):
    ...
    #hook_handle = model.layer2.register_forward_hook(hook)  # Assuming layer2 is an earlier convolutional layer
...
for epoch in range(num_epochs):
    ...
    if epoch == num_epochs - 1:
        ...
        plot_grad_cam_for_image(model, image_path=sample_image_path51, actual_class=0, target_class=0, filename='NewOutputs/resnet50_untrain/grad_cam51.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path2419, actual_class=1, target_class=1, filename='NewOutputs/resnet50_untrain/grad_cam2419.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path2373, actual_class=1, target_class=1, filename='NewOutputs/resnet50_untrain/grad_cam2373.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path939, actual_class=0, target_class=0, filename='NewOutputs/resnet50_untrain/grad_cam939.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path331, actual_class=0, target_class=0, filename='NewOutputs/resnet50_untrain/grad_cam331.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path370, actual_class=1, target_class=1, filename='NewOutputs/resnet50_untrain/grad_cam370.png')


    # Save Confusion Matrix as Image after the last epoch
    if epoch == num_epochs - 1:
        plot_confusion_matrix(conf_matrix, classes=[str(i) for i in range(2)], filename='NewOutputs/resnet50_untrain/confusion_matrix_resnet50_untrained.png')
