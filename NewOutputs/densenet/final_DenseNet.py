...
# Including the file path for the output
output_file = 'NewOutputs/densenet/brain_tumor_classification_DenseNet_trained.txt'

sys.stdout = open(output_file, 'w')


# Checking if CUDA GPU is available, then print it, otherwise go for cpu
...
# Function to get a subset of dataset indices 
...
def plot_roc_curve_and_output_auc(labels, probs, filename_png='NewOutputs/densenet/ROC_curve_densenet_trained.png', filename_csv='NewOutputs/densenet/ROC_data_densenet_trained.csv'):
...
# Defining the root directory containing the "brain tumor" and "healthy" folders
root_dir = r"C:\Users\Gigabyte\Downloads\Brain Tumor Data Set\Brain Tumor Data Set"

# Transform dataset input to match inputs for densenet model
...
for epoch in range(num_epochs):
    ...
    if epoch == num_epochs - 1:
        ...
        plot_grad_cam_for_image(model, image_path=sample_image_path51, actual_class=0, target_class=0, filename='NewOutputs/densenet/grad_cam51.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path2419, actual_class=1, target_class=1, filename='NewOutputs/densenet/grad_cam2419.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path2373, actual_class=1, target_class=1, filename='NewOutputs/densenet/grad_cam2373.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path939, actual_class=0, target_class=0, filename='NewOutputs/densenet/grad_cam939.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path331, actual_class=0, target_class=0, filename='NewOutputs/densenet/grad_cam331.png')
        plot_grad_cam_for_image(model, image_path=sample_image_path370, actual_class=1, target_class=1, filename='NewOutputs/densenet/grad_cam370.png')
    # Save Confusion Matrix as Image after the last epoch
    if epoch == num_epochs - 1:
        plot_confusion_matrix(conf_matrix, classes=[str(i) for i in range(2)], filename='NewOutputs/densenet/confusion_matrix_DenseNet_trained.png')
